...
import { uploadGoogleFile, deleteGoogleFile } from "../../gcp/storage"
...
app.delete('/', async (c) => {
  ...
  const result = await deleteGoogleFile(name)
  return c.json({ ok: true, message: result ? 'Delete success' : 'Delete failed' })
})
